...
function fn_getQuestionList() {
	...
	mz.util.ajaxObj('/api/sp/demo/question', '', 'GET', 'json', fn_getQuestionList_callback, false);
}
...
function fn_getQuestion(koId) {
	...
	mz.util.ajaxObj('/api/sp/demo/question/' + koId, '', 'GET', 'json', fn_getQuestion_callback, false);
...
function fn_getPreviousResult() {
	...
	// 지난 결과
	mz.util.ajaxObj('/api/sp/demo/answer/' + koId + '/pre', '', 'GET', 'json', fn_getPreviousResult_callback, false);
...
function fn_evaluate() {
	...
	setTimeout(function() {
		mz.util.ajaxObj('/api/sp/demo/evaluate', JSON.stringify(data), 'POST', 'json', fn_evaluate_callback, false);
	}, 500);
}
...
function fn_getFile() {
	...
	if ( !isPlaying ) {
		...
		mz.util.ajaxObj('/api/sp/demo/files', data, 'GET', 'json', fn_getFile_callback,  false);
	}
}
